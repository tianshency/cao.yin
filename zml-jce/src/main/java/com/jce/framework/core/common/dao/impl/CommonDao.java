...
import com.jce.framework.core.common.dao.ICommonDao;
...
import com.jce.framework.core.extend.swftools.SwfToolsUtil;
import com.jce.framework.core.extend.template.DataSourceMap;
import com.jce.framework.core.extend.template.Template;
import com.jce.framework.core.util.*;
...
//				FileCopyUtils.copy(mf.getBytes(), savefile);
	...
	public List<Map<String, String>> uploadFile(UploadFile uploadFile, String dirName, String id) {
		...
		List rsList = new ArrayList();
 		try {
 			if(id == null){
 				id = DateUtils.getDataString(DateUtils.yyyymmddhhmmss)+StringUtil.random(8);//自定义文件名称
 			}
			...
			for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
				...
				if (mf != null) {
					...
					try {
						...
						String[] filename = mf.getOriginalFilename().split("\\.");
						...
						coverPath =  dirName + "/" + pathParam + "/"+ id;
						rsPath = coverPath + "/min." + suffix;
						path += coverPath;
						//存大图
						...
						FileUtils.writeImgFile(mf, mini, 100, 0);
					} catch (Exception e) {
						e.printStackTrace();
					}
					rsMap.put("rsPath", rsPath);
					coverFalg = false;
				}
				rsList.add(rsMap);
			}
