package com.jce.framework.web.cgform.controller.autoform;
...
import com.jce.framework.core.util.DynamicDBUtil;
import com.jce.framework.core.util.MyBeanUtils;
import com.jce.framework.core.util.ResourceUtil;
import com.jce.framework.core.util.SqlUtil;
...
import com.jce.framework.web.cgform.entity.autoform.AutoFormEntity;
import com.jce.framework.web.cgform.entity.autoform.AutoFormParamEntity;
import com.jce.framework.web.cgform.entity.autoform.AutoFormStyleEntity;
import com.jce.framework.web.cgform.service.autoform.AutoFormDbServiceI;
import com.jce.framework.web.cgform.service.autoform.AutoFormServiceI;
import com.jce.framework.web.cgform.util.AutoFormCommUtil;
import com.jce.framework.web.cgform.util.AutoFormTemplateParseUtil;
import com.jce.framework.web.cgform.util.TemplateUtil;
...
public class AutoFormController extends BaseController {
	...
	public ModelAndView autoForm(HttpServletRequest request) {
		return new ModelAndView("sys/cgform/autoform/autoFormList");
	}
	...
	public void datagrid(AutoFormEntity autoForm,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		//查询条件组装器
		com.jce.framework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, autoForm, request.getParameterMap());
		try{
		//自定义追加查询条件
		}catch (Exception e) {
...
//		List<AutoFormStyleEntity> list = this.systemService.findHql(hql);
		...
		return new ModelAndView("sys/cgform/autoform/autoForm-add");
	}
	/**
	 * 表单表编辑页面跳转
	 * 
	...
	public ModelAndView goUpdate(AutoFormEntity autoForm, HttpServletRequest req) {
		...
		return new ModelAndView("sys/cgform/autoform/autoForm-update");
	}
	...
	public ModelAndView parse(AutoFormEntity autoForm, HttpServletRequest req) {
		...
		return new ModelAndView("sys/cgform/autoform/autoForm-view");
	}
	...
	public ModelAndView viewContent(AutoFormEntity autoForm, HttpServletRequest req) {
		...
		try {
			...
			if(StringUtils.isNotBlank(autoForm.getFormName())){
				...
				if(autoForm==null){
					return new ModelAndView("sys/cgform/autoform/autoForm-error").addObject("message", "表单信息获取失败！");
				}
			}else{
				return new ModelAndView("sys/cgform/autoform/autoForm-error").addObject("message", "formName不能为空！");
			}
			...
			if(paramList!=null&&paramList.size()>0){
				for(String param:paramList){
					...
					if(paramValue==null){
						return new ModelAndView("sys/cgform/autoform/autoForm-error").addObject("message", "缺少表单参数："+param);
					}
				}
			}
			...
			if(AutoFormTemplateParseUtil.OP_VIEW.equals(op)){
				...
				return new ModelAndView("sys/cgform/autoform/autoForm-review-add");
				...
				req.setAttribute("formContent", ftlContent);
				req.setAttribute("op", op);
				return new ModelAndView("sys/cgform/autoform/autoForm-review-update");
				...
				req.setAttribute("formContent", ftlContent);
				req.setAttribute("op", op);
				return new ModelAndView("sys/cgform/autoform/autoForm-review-addorupdate");
			}
			req.setAttribute("formContent", ftlContent);
			req.setAttribute("op", op);
			return new ModelAndView("sys/cgform/autoform/autoForm-review");
		} catch (Exception e) {
			e.printStackTrace();
			message = "表单添加失败："+e.getMessage();
		} 
		return new ModelAndView("sys/cgform/autoform/autoForm-error").addObject("message", message);
	}
