package com.jce.framework.web.cgform.controller.generate;
...
import com.jce.framework.core.enums.OnlineGenerateEnum;
...
import com.jce.framework.web.cgform.entity.enhance.CgformEnhanceJavaEntity;
import com.jce.framework.web.cgform.entity.enhance.CgformEnhanceJsEntity;
import com.jce.framework.web.cgform.entity.generate.GenerateEntity;
import com.jce.framework.web.cgform.entity.generate.GenerateSubListEntity;
import com.jce.framework.web.cgform.service.build.DataBaseService;
import com.jce.framework.web.cgform.service.button.CgformButtonServiceI;
...
import com.jce.framework.web.cgform.service.impl.generate.TempletContextWord;
...
public class GenerateController extends BaseController {
	...
	public ModelAndView gogenerate( CgFormHeadEntity cgFormHead,HttpServletRequest request) {
		...
		if(cgFormHead.getJformType()==1 || cgFormHead.getJformType()==3){
			...
			returnModelAndView = "sys/cgform/generate/single";
		}else{
			//如果是主表，则进入一对多模型
			...
			returnModelAndView = "sys/cgform/generate/one2many";
		}
			...
			if(tableexist){
				...
				if(modeEnum!=null){
					if("system".equals(modeEnum.getVersion())){
						...
						CgformCodeGenerate generate = null;
						//=======
					...
					j.setMsg(ftlDescription+"：功能生成成功，请刷新项目重启，菜单访问路径： bobo update");
					//===========
				}else{
		...
		try{
			...
			*/
			...
			if(modeEnum!=null){
				...
				j.setMsg(codeParamEntityIn.getFtlDescription()+"：功能生成成功，请刷新项目重启，菜单访问路径：bobo update");
			}else{
				j.setMsg("代码生成器不支持该页面风格");
			}
	...
	public ModelAndView goFileTree(HttpServletRequest request) {
		return new ModelAndView("sys/cgform/generate/fileTree");
	}
	/**
	 * 返回子目录json
	 * @param parentNode
