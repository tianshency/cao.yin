...
public class CgAutoListController extends BaseController{
		...
		for(CgFormFieldEntity b:beans){
			...
			if(dicList.size() > 0){
				for(Map<String, Object> resultMap:result){
					...
					System.out.println("FieldName====" + b.getFieldName().toString());
	...
	private void dealDic(List<Map<String, Object>> result,
		...
		for(CgFormFieldEntity bean:beans){
			...
			//bobo
			String dicParam = bean.getDictParam();//字典参数 
			if(StringUtil.isEmpty(dicTable)&& StringUtil.isEmpty(dicCode)){
				...
				if(!bean.getShowType().equals("popup")){
					...
					//bobo2016-12-31List<DictEntity> dicDataList = queryDicByParam(dicTable, dicCode,dicText, dicParam);
					for(Map r:result){
...
//			}
	...
	private void loadDic(Map m, CgFormFieldEntity bean) {
		...
		if(!bean.getShowType().equals("popup")){
			...
			//bobo2016-12-31 List<DictEntity> dicDatas = queryDicByParam(dicT, dicF,dicText, dicParam);
...
//		List<Map<String, Object>> dicDatas = systemService.findForJdbc(dicSql.toString());
	 ...
	 * @return
	 */
	private List<DictEntity> queryDicByParam(String dicTable, String dicCode,String dicText,String dicParam) {
		return systemService.queryDictByParam(dicTable, dicCode, dicText, dicParam);
	}
