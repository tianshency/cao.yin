...
@Service("zmlUserReleaseFoodService")
@Transactional
public class ZmlUserReleaseFoodServiceImpl extends CommonServiceImpl implements ZmlUserReleaseFoodServiceI {
 		...
 		try {
			...
			if (imgFile != null) {
				...
				try {
					...
					coverPath = Constant.IMG_FILE_PATH_ONE + DocumentDirName.RELEASE_FOOD.getStatusValue() + "/" + pathParam + "/"+ id;
					entity.setCoverImg(coverPath + "/min." + suffix);
					path = path + coverPath;
					//存大图
			...
			if (files != null) {
				...
				for (int i=0; i< files.size(); i++) {
					...
					if (file != null) {
						...
						String tempPath =    DocumentDirName.RELEASE_FOOD.getStatusValue() + "/" + yyyyMM + "/"
								+ id + "/";
	 ...
	 * @return
	 */
	private void doAddBus(ZmlUserReleaseFoodEntity t) throws Exception{
 	}
 	/**
	 * 更新操作增强业务
	 * @param t
	 * @return
	 */
	private void doUpdateBus(ZmlUserReleaseFoodEntity t) throws Exception{
 	}
 	/**
	 * 删除操作增强业务
	 * @param id
	 * @return
	 */
	private void doDelBus(ZmlUserReleaseFoodEntity t) throws Exception{
 	}
 	
 	private Map<String,Object> populationMap(ZmlUserReleaseFoodEntity t){
		...
		map.put("kind", t.getKind());
		map.put("price", t.getPrice());
		map.put("unit", t.getUnit());
		map.put("moisture", t.getMoisture());
		map.put("mildew", t.getMildew());
		map.put("bulk_density", t.getBulkDensity());
		map.put("food_type", t.getFoodType());
		map.put("filter_type", t.getFilterType());
		map.put("particular_year", t.getParticularYear());
		map.put("total_num", t.getTotalNum());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,ZmlUserReleaseFoodEntity t){
 		...
 		sql  = sql.replace("#{kind}",String.valueOf(t.getKind()));
 		sql  = sql.replace("#{price}",String.valueOf(t.getPrice()));
 		sql  = sql.replace("#{unit}",String.valueOf(t.getUnit()));
 		sql  = sql.replace("#{moisture}",String.valueOf(t.getMoisture()));
 		sql  = sql.replace("#{mildew}",String.valueOf(t.getMildew()));
 		sql  = sql.replace("#{bulk_density}",String.valueOf(t.getBulkDensity()));
 		sql  = sql.replace("#{food_type}",String.valueOf(t.getFoodType()));
 		sql  = sql.replace("#{filter_type}",String.valueOf(t.getFilterType()));
 		sql  = sql.replace("#{particular_year}",String.valueOf(t.getParticularYear()));
	...
	public List<Map<String, Object>> findDatagrid(ZmlUserReleaseFoodEntity entity, DataGrid dataGrid) {
		...
		String sqlCnt = "select count(1) from zml_user_release_food t";
		if (!sqlWhere.isEmpty()) {
		...
		// 取出当前页的数据 
		String sql = "select t.id,t.user_id userId,t.create_name userName,t.type, t.title,t.total_num totalNum,t.kind,t.cover_img coverImg,t.valid_start_date validStartDate,t.valid_end_date validEndDate " +
		" ,t.update_date updateDate,t.create_date createDate,t.view_count viewCount,t.status " 
		+" from zml_user_release_food t ";
		if (!sqlWhere.isEmpty()) {
			sql += " where" + sqlWhere;
		}
		//查询非删除状态的数据
	...
	// 拼查询条件（where语句）
	private String getSqlWhere(ZmlUserReleaseFoodEntity pageObj) {
		// 拼出条件语句
