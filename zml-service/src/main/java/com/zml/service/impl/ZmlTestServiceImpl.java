...
@Service("zmlTestService")
@Transactional
public class ZmlTestServiceImpl extends CommonServiceImpl implements ZmlTestServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(ZmlTestEntity t) throws Exception{
 	}
 	/**
	 * 更新操作增强业务
	 * @param t
	 * @return
	 */
	private void doUpdateBus(ZmlTestEntity t) throws Exception{
 	}
 	/**
	 * 删除操作增强业务
	 * @param id
	 * @return
	 */
	private void doDelBus(ZmlTestEntity t) throws Exception{
 	}
 	
 	private Map<String,Object> populationMap(ZmlTestEntity t){
		...
		map.put("tuijianren", t.getTuijianren());
		map.put("age", t.getAge());
		map.put("sex", t.getSex());
		map.put("phone", t.getPhone());
		map.put("email", t.getEmail());
		map.put("touxiang", t.getTouxiang());
		map.put("jianli", t.getJianli());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,ZmlTestEntity t){
 		...
 		sql  = sql.replace("#{tuijianren}",String.valueOf(t.getTuijianren()));
 		sql  = sql.replace("#{age}",String.valueOf(t.getAge()));
 		sql  = sql.replace("#{sex}",String.valueOf(t.getSex()));
 		sql  = sql.replace("#{phone}",String.valueOf(t.getPhone()));
 		sql  = sql.replace("#{email}",String.valueOf(t.getEmail()));
 		sql  = sql.replace("#{touxiang}",String.valueOf(t.getTouxiang()));
 		sql  = sql.replace("#{jianli}",String.valueOf(t.getJianli()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				if("class".equals(cgJavaType)){
					//因新增时已经校验了实例化是否可以成功，所以这块就不需要再做一次判！
